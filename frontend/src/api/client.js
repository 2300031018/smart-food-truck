...
export const api = {
  ...
  // Admin utilities
  cleanupOrphans: (token) => request('/admin/cleanup-orphans', { method: 'POST', token }),
  ...
  getManagersHierarchy: (token, { includeInactive=false } = {}) => {
    const qs = includeInactive ? '?includeInactive=true' : '';
    return request(`/users/managers/hierarchy${qs}`, { token });
  },
  ...
  deleteStaff: (token, id) => request(`/users/staff/${id}`, { method: 'DELETE', token })
};
